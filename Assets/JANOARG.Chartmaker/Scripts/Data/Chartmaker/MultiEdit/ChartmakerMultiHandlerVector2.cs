...
{
    public class ChartmakerMultiHandlerVector2: ChartmakerLerpableMultiHandler<Vector2>
    {
        public int Axis = 0;

        public override ExpressionContext GetExpressionContext()
        {
            return new ExpressionContext().SetVariable("x", 0).SetVariable("y", 0).SetVariable("i", 0).SetVariable("t", 0);
        }
    
        public override Vector2 Get(Vector2 from, object source, int index = 0)
        {
