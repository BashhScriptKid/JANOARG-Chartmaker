...
public class QuickBuild
{
    ...
    public static void Build()
    {
        ...
        // Build for Android
        DoBuildPlayer(new BuildPlayerOptions()
        {
    ...
    static void DoBuildPlayer(BuildPlayerOptions options)
    {
        try
        {
            EditorUtility.DisplayProgressBar("Building", "Building for " + options.target, 0);
            var report = BuildPipeline.BuildPlayer(options);
            if (report.summary.result != UnityEditor.Build.Reporting.BuildResult.Succeeded)
            {
                throw new System.Exception("There was an error trying to build for " + options.target);
            }
        }
        finally
        {
